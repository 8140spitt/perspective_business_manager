...
export function parseHex(hex: string): Rgb {
  ...
  if (/^[0-9a-fA-F]{3}$/.test(clean)) clean = clean.split('').map((x) => x + x).join('');
...
export function rgbToHex(rgb: Rgb): Hex {
  ...
  return `#${to(rgb.r)}${to(rgb.g)}${to(rgb.b)}` as Hex;
}

export function rgbTo01(rgb: Rgb): Rgb01 {
  return { r: rgb.r / 255, g: rgb.g / 255, b: rgb.b / 255 };
}

export function rgbFrom01(rgb: Rgb01): Rgb {
  return { r: clamp255(rgb.r * 255), g: clamp255(rgb.g * 255), b: clamp255(rgb.b * 255) };
}

export function srgbToLinearChannel(x: number) {
  x = clamp(x);
  return x <= 0.04045 ? x / 12.92 : ((x + 0.055) / 1.055) ** 2.4;
}

export function linearToSrgbChannel(x: number) {
  x = clamp(x);
  return x <= 0.0031308 ? 12.92 * x : 1.055 * x ** (1 / 2.4) - 0.055;
}

export function relativeLuminance(hexOrRgb: string | Rgb): number {
...
export function visualBrightness(hexOrRgb: string | Rgb): number {
  const rgb = typeof hexOrRgb === 'string' ? parseHex(hexOrRgb) : hexOrRgb;
  const r = rgb.r / 255;
  const g = rgb.g / 255;
  const b = rgb.b / 255;
...
export function bestTextColour(bg: string | Rgb): '#111111' | '#ffffff' {
