<script lang="ts">
	...
	let palette = $derived(generatePalette(seed, {
		stopSelection,
		anchor,
		profile,
		tintStrength,
...
<section class="tester">
	...
	<div class="sliders">
		<label>Tint <input type="range" min="0.5" max="1.5" step="0.01" bind:value={tintStrength} /> {n(tintStrength, 2)}</label>
		<label>Shade <input type="range" min="0.5" max="1.5" step="0.01" bind:value={shadeStrength} /> {n(shadeStrength, 2)}</label>
		<label>Tone <input type="range" min="0" max="2" step="0.01" bind:value={toneStrength} /> {n(toneStrength, 2)}</label>
		<label>Vibrancy <input type="range" min="0.5" max="1.5" step="0.01" bind:value={vibrancy} /> {n(vibrancy, 2)}</label>
	</div>

	<div class="summary">
		<div><strong>Engine</strong> Bezier visual-brightness paint path</div>
		<div><strong>Anchor</strong> {activeAnchor}</div>
		<div class:pass={quality.pass} class:fail={!quality.pass}><strong>Quality</strong> {quality.score}/100</div>
	</div>
		...
		{#each palette as stop}
			<div class:anchor={stop.anchor} class="swatch">
				...
				<div class="meta">
					...
					<span>mix W {n(stop.mix.white * 100, 1)} B {n(stop.mix.black * 100, 1)} C {n(stop.mix.colour * 100, 1)}</span>
...
<style>
	.tester { display: grid; gap: 1.5rem; padding: 1.5rem; font-family: system-ui, sans-serif; }
	.controls { display: flex; flex-wrap: wrap; align-items: end; gap: 1rem; }
	label { display: grid; gap: 0.35rem; font-weight: 700; }
	input[type='text'], input[type='number'], select { padding: 0.5rem 0.65rem; border: 1px solid #ccc; border-radius: 0.5rem; }
	input[type='color'] { width: 4rem; height: 2.5rem; }
	code { padding: 0.55rem 0.7rem; border-radius: 0.5rem; background: #f3f4f6; font-weight: 700; }
	.sliders { display: grid; gap: .65rem; max-width: 48rem; }
	.sliders label { grid-template-columns: 6rem 1fr 3rem; align-items: center; }
	.summary { display: flex; flex-wrap: wrap; gap: .75rem; }
	.summary > div { width: fit-content; padding: .6rem .8rem; border-radius: .6rem; background: #f3f4f6; }
	.pass { background: #dcfce7 !important; color: #14532d; }
	.fail { background: #fee2e2 !important; color: #7f1d1d; }
	.issues { margin: 0; padding-left: 1.25rem; color: #7f1d1d; }
	.ramp { display: grid; grid-template-columns: repeat(auto-fit, minmax(10rem, 1fr)); gap: 1rem; }
	.swatch { overflow: hidden; border: 1px solid #ddd; border-radius: .75rem; background: white; }
	.swatch.anchor { outline: 3px solid #111; }
	.chip { display: grid; min-height: 5rem; place-items: center; font-size: 1.25rem; font-weight: 900; }
	.meta { display: grid; gap: .2rem; padding: .75rem; font-size: .72rem; }
	pre { overflow: auto; padding: 1rem; border-radius: .75rem; background: #111; color: #eee; }
</style>
