...
export function getPaletteProfile(profile = 'vivid') {
    return { ...PROFILE[profile] };
}
export function getBrightnessTargets(overrides) {
    return { ...TARGET_BRIGHTNESS, ...(overrides ?? {}) };
}
export function calculateStopFromColor(seed, targets = TARGET_BRIGHTNESS) {
...
function resolveOptions(options) {
    ...
    return {
        ...
        stopSelection: options.stopSelection ?? 'auto',
...
function seedKind(rgb) {
    ...
    if (oklch.C < 0.045 || colourStrength < 12)
        return 'near-neutral';
    return 'chromatic';
}
function neutralAtBrightness(target) {
...
function solveHslLightnessForBrightness(h, s, target) {
    let lo = 0;
    let hi = 1;
    for (let i = 0; i < 32; i++) {
        const mid = (lo + hi) / 2;
        const rgb = hslToRgb({ h, s: s * 100, l: mid * 100 });
        if (visualBrightness(rgb) < target)
            lo = mid;
        else
            hi = mid;
    }
...
function generateChromaticStop(seed, step, anchor, target, opts) {
    const seedHsl = rgbToHsl(seed);
    const seedHue = seedHsl.h;
    const seedSat = clamp(seedHsl.s / 100, 0, 1);
    if (step === anchor)
        return seed;
    const side = step < anchor ? 'light' : 'dark';
...
function generateNearNeutralStop(seed, step, anchor, target, opts) {
    const seedHsl = rgbToHsl(seed);
...
export function generatePalette(seedHex, options = {}) {
...
export function assessPaletteQuality(palette, options = {}) {
    ...
    if (badHex.length)
        issues.push(`${badHex.length} invalid hex output(s).`);
    ...
    if (options.seed) {
        ...
        if (!preserved)
            issues.push(`Seed was not preserved at ${anchor}.`);
    ...
    if (duplicateCount)
        warnings.push(`${duplicateCount} duplicate stop colour(s).`);
    ...
    for (let i = 1; i < palette.length; i++) {
        if (palette[i].brightness > palette[i - 1].brightness + 0.035)
            ordered = false;
    }
    if (!ordered)
        warnings.push('Brightness order has visible reversals.');
    ...
    if (hugeJump)
        warnings.push('One or more adjacent stops have a large perceptual jump.');
