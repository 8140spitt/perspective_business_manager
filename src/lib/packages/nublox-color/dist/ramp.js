...
export function generateRamp(hex, options = {}) {
    ...
    for (const stop of STOPS) {
        ...
        if (fitted.compressed)
            notes.push('Chroma compressed to fit sRGB gamut.');
        if (Math.abs(hue.adjustment) > .5)
            notes.push('Small hue compensation applied for perceptual consistency.');
        if (contrastRatio(fitted.rgb, white) >= 4.5 || contrastRatio(fitted.rgb, black) >= 4.5)
            notes.push('Has at least one WCAG AA normal-text foreground among black/white.');
        const item = {
            stop: stop,
            hex: hexOut,
